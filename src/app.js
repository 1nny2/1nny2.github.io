...
const CONFIG = {
    ...
    ringParticleCount: lightweight ? 1800 : 6000,
    starCount: lightweight ? 1400 : 4000,
...
let manualUntil = 0;
document.getElementById('help-panel').open = !compactDevice;
...
function buildNebula() {
    ...
    const sprites = [];
    for (let i = 0; i < (lightweight ? 5 : 10); i++) {
...
function updateCamera(dt) {
    ...
    if (Math.abs(cameraState.handRotation) > 0.1) {
        cameraState.phi += cameraState.handRotation * CONFIG.rotationSpeedMultiplier * step;
    }
    ...
    // 闲置自转
    if (!reducedMotion && !pointers.size && performance.now() > manualUntil && !cameraState.isFist && !cameraState.isOpen && !cameraState.twoHandActive && Math.abs(cameraState.handRotation) < 0.1) {
    ...
    if (fxState.collapse > 0) {
        camera.position.multiplyScalar(1 - 0.0008 * fxState.collapse * 60 * dt);
    }
}
